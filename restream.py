...
# --- MAILCAM helper: "Delivery services seen" (reset daily at 03:00) ---
_seen_today = {}
_last_reset_key = None
def _delivery_today_lines():
    import time
    global _seen_today, _last_reset_key, cur_meta
    t   = time.localtime()
    key = (t.tm_year, t.tm_yday, t.tm_hour >= 3)  # flips at 03:00 local
    if key != _last_reset_key:
        _seen_today = {}
        _last_reset_key = key
    try:
        flags = (cur_meta.get("delivery") or {})
        present = [k.upper() for k, v in flags.items() if v]
        ts = time.strftime("%m-%d-%y %H:%M", t)
        for svc in present:
            _seen_today.setdefault(svc, ts)
    except Exception:
        pass
    lines=[]
    for svc, ts in sorted(_seen_today.items(), key=lambda kv: kv[1]):
        lines.append(svc); lines.append(ts)
    return lines
# --- end MAILCAM helper ---
...
def detect_and_render():
    ...
    try:
        font = ImageFont.truetype("/usr/share/fonts/truetype/inter/Inter-Regular.ttf", 22)
        font_big = ImageFont.truetype("/usr/share/fonts/truetype/inter/Inter-Regular.ttf", 22)
    except Exception:
        font = ImageFont.truetype("DejaVuSans.ttf", 22); font_big = font

    COCO_KEEP = {"person","car","truck","bus","bicycle","motorcycle"}
    while True:
        t0=time.time()
        try:
            r = requests.get(SRC, timeout=0.9); r.raise_for_status()
        ...
        header_lines = ["DELIVERIES & PICKUPS:"]

        # --- draw delivery today list ---
        dl = _delivery_today_lines()
        panel_title = "DELIVERIES & PICKUPS"
        # Draw title and entries directly with PIL (no put_text)
        try:
            font_big2 = ImageFont.truetype("/usr/share/fonts/truetype/inter/Inter-Regular.ttf", 28)
        except Exception:
            font_big2 = font
        x0 = VID_W + 10
        y0 = 8
        draw.text((x0, y0), panel_title, fill=(255,255,255), font=font_big2); y0 += 30
        for ln in dl:
            draw.text((x0, y0), ln, fill=(230,230,230), font=font); y0 += 32
        # --- end delivery list ---
        ...
        # --- end delivery list ---
            ...
            header_lines += ["No deliveries", ts_now]

        try: font_big2 = ImageFont.truetype("/usr/share/fonts/truetype/inter/Inter-Regular.ttf", 28)
        except: font_big2 = font
        line_h =    26
        top_pad, bottom_pad, side_pad = 8, 8, 10
        block_h = max(top_pad + bottom_pad + line_h*len(header_lines), 38)
        color = (48,209,88) if present else (99,99,102)
        ...
        cur_meta={
            ...
            "delivery": {k: (k in present_set) for k in DELIVERY_KEYS},
        ...
        buf=io.BytesIO(); canvas.save(buf, format="JPEG", quality=25, subsampling=2); cur_frame=buf.getvalue()
        dt=time.time()-t0; time.sleep(max(0.0, float(os.getenv("MAILCAM_PERIOD","2.2"))-dt))
# ===== API =====
def mjpeg_gen():
    boundary="frame"
    while True:
...
# --- helper: "Delivery services seen" state (reset daily at 03:00) ---
_seen_today = {}          # { "AMAZON": "09-26-25 13:35", ... }
_last_reset_key = None    # (year, yday, after3)

def _delivery_today_lines():
    import time
    global _seen_today, _last_reset_key, cur_meta
    t   = time.localtime()
    key = (t.tm_year, t.tm_yday, t.tm_hour >= 3)  # flips after 03:00 each day
    if key != _last_reset_key:
        _seen_today = {}
        _last_reset_key = key

    # Update "seen today" from current in-frame delivery flags (if available)
    try:
        curflags = (cur_meta.get("delivery") or {})
        present  = [k.upper() for k,v in curflags.items() if v]
        ts       = time.strftime("%m-%d-%y %H:%M", t)
        for svc in present:
            _seen_today.setdefault(svc, ts)
    except Exception:
    ...
    # Return as two-line entries: SERVICE on one line, timestamp next line
    lines = []
    for svc, ts in sorted(_seen_today.items(), key=lambda kv: kv[1]):
        lines.append(svc)
        lines.append(ts)
